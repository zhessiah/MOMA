...
class EA_QPLEX_curiosity_vdn_Learner:
    def __init__(self, mac, scheme, logger, args, groups=None):
        ...
        if args.EA:
            ...
            if args.mixer is not None:
                if args.mixer == "dmaq":
                    self.EA_mixer = DMAQer(args)
                elif args.mixer == 'dmaq_qatten':
                    self.EA_mixer = DMAQ_QattenMixer(args)
                else:
                    raise ValueError("Mixer {} not recognised.".format(args.mixer))
                self.EA_params = list(self.EA_mixer.parameters())
                self.EA_target_mixer = copy.deepcopy(self.EA_mixer)

            self.EA_optimiser = RMSprop(params=self.EA_params, lr=args.lr, alpha=args.optim_alpha, eps=args.optim_eps)
    ...
    def sub_train(self, batch: EpisodeBatch, all_teams, t_env: int, episode_num: int, mac,
                  mixer,
                  optimiser, params,
                  intrinsic_rewards,
        ...
        for t in range(batch.max_seq_length):
            hidden_states.append(mac.hidden_states.view(batch.batch_size, self.args.n_agents, -1))
        ...
        if mixer is not None:
            if self.args.mixer == "dmaq_qatten":
                ...
                if self.args.graphmixer:
                    ...
                    ans_adv, _, _, _, _, = mixer(
                        chosen_action_qvals,
                        batch["state"][:, :-1], actions=actions_onehot, max_q_i=max_action_qvals, is_v=False,
                        agent_obs=batch["obs"][:,
                                  :-1],
                        team_rewards=rewards,
                        hidden_states=hidden_states[
                    ...
                    local_targets = local_rewards + self.args.gamma * (1 - terminated).repeat(1, 1,
                                                                                              self.args.n_agents) \
                    ...
                    # total loss
                    lambda_local = 1
                    graphmixloss = lambda_local * local_loss
                else:
            ...
            if self.args.double_q:
                if self.args.mixer == "dmaq_qatten":
                    if self.args.graphmixer:
                        target_chosen, _, _, _, _ = self.target_mixer(target_chosen_qvals,
                                                                      batch["state"][:, 1:], is_v=True,
                                                                      agent_obs=batch["obs"][:, 1:],
                                                                      hidden_states=target_hidden_states
                                                                      )
                        target_adv, _, _, _, _ = self.target_mixer(target_chosen_qvals,
                                                                   batch["state"][:, 1:],
    ...
    def sub_train_EA(self, batch: EpisodeBatch, all_teams, t_env: int, episode_num: int, mac,
                     mixer,
                     optimiser, params,
                     intrinsic_rewards,
                     show_demo=False, save_data=None, show_v=False, save_buffer=False, ec_buffer=None):
        ...
        selected_team.cuda()
        selected_team.agent.train()
        selected_team.init_hidden(batch.batch_size)
        # Calculate estimated Q-Values

        mac_out = selected_team.forward(batch, batch.max_seq_length, batch_inf=True)
        hidden_states = []
        for t in range(batch.max_seq_length):
        ...
        # Calculate the Q-Values necessary for the target
        index = random.sample(list(range(self.args.pop_size + 1)), 1)[0]
        selected_team_target = all_teams[index]
        selected_team_target.cuda()
        selected_team_target.agent.train()
        selected_team_target.init_hidden(batch.batch_size)
        target_mac_out = selected_team_target.forward(batch, batch.max_seq_length, batch_inf=True)[:, 1:, ...]
        target_hidden_states = []
        for t in range(batch.max_seq_length):
            target_hidden_states.append(
                selected_team_target.hidden_states.view(batch.batch_size, self.args.n_agents, -1))
        target_hidden_states = th.stack(target_hidden_states[1:], dim=1)
        # Mask out unavailable actions
        target_mac_out[avail_actions[:, 1:] == 0] = -9999999
        ...
        if self.args.double_q:
            ...
            selected_team_target.init_hidden(batch.batch_size)
            for t in range(batch.max_seq_length):
                target_agent_outs = selected_team_target.forward(batch, t=t)
                target_mac_out.append(target_agent_outs)
            # We don't need the first timesteps Q-Value estimate for calculating targets
            target_mac_out = th.stack(target_mac_out[1:], dim=1)  # Concat across time
            target_max_qvals = target_mac_out.max(dim=3)[0]
        ...
        if mixer is not None:
            if self.args.mixer == "dmaq_qatten":
                if self.args.graphmixer:
                    ans_chosen, q_attend_regs, head_entropies, local_rewards, alive_agents_mask = mixer(
                        chosen_action_qvals,
                        batch["state"][:, :-1], is_v=True, EA=True,
                        agent_obs=batch["obs"][:,
                                  :-1],
                        team_rewards=rewards,
                        hidden_states=hidden_states[
                    ...
                    ans_chosen, q_attend_regs, head_entropies = \
                        mixer(chosen_action_qvals, batch["state"][:, :-1], is_v=True, EA=True)
                    ans_adv, _, _ = mixer(chosen_action_qvals, batch["state"][:, :-1], actions=actions_onehot,
                                          max_q_i=max_action_qvals, is_v=False, EA=True)
                chosen_action_qvals = ans_chosen + ans_adv

            else:
                ans_chosen = mixer(chosen_action_qvals, batch["state"][:, :-1], is_v=True, EA=True)
                ans_adv = mixer(chosen_action_qvals, batch["state"][:, :-1], actions=actions_onehot,
                                max_q_i=max_action_qvals, is_v=False, EA=True)
                chosen_action_qvals = ans_chosen + ans_adv
            ...
            if self.args.double_q:
                if self.args.mixer == "dmaq_qatten":
                    if self.args.graphmixer:
                        ...
                        target_adv, _, _, _, _ = self.target_mixer(target_chosen_qvals,
                                                                   batch["state"][:, 1:],
                                                                   actions=cur_max_actions_onehot,
                                                                   max_q_i=target_max_qvals, is_v=False, EA=True,
                                                                   agent_obs=batch["obs"][:, 1:],
                                                                   hidden_states=target_hidden_states
                                                                   )
                    else:
                ...
                target_max_qvals = self.target_mixer(target_max_qvals, batch["state"][:, 1:], is_v=True, EA=True)
        ...
        # Normal L2 loss, take mean over actual data
        if self.args.mixer == "dmaq_qatten":
            ea_loss = (masked_td_error ** 2).sum() / mask.sum() + q_attend_regs
            if self.args.use_emdqn:
                emdqn_loss = (emdqn_masked_td_error ** 2).sum() / mask.sum() * self.args.emdqn_loss_weight
                ea_loss += emdqn_loss
        else:
    ...
    def train(self, batch: EpisodeBatch, all_teams, t_env: int, episode_num: int,
        ...
        intrinsic_rewards = self.vdn_learner.train(batch, t_env, episode_num, save_buffer=False, imac=self.mac,
                                                   timac=self.target_mac)
        if self.args.is_prioritized_buffer:
            masked_td_error, mask = self.sub_train(batch, all_teams, t_env, episode_num, self.mac,
                                                   self.mixer, self.optimiser,
                                                   self.params, intrinsic_rewards=intrinsic_rewards,
                                                   show_demo=show_demo, save_data=save_data, show_v=show_v,
                                                   ec_buffer=ec_buffer)
        else:
            if (episode_num % self.args.EA_freq) == 0 and self.args.EA:
                self.sub_train_EA(batch, all_teams, t_env, episode_num, self.mac, self.mixer, self.optimiser,
                                  self.params,
                                  intrinsic_rewards=intrinsic_rewards,
                                  show_demo=show_demo, save_data=save_data, show_v=show_v, ec_buffer=ec_buffer)
            self.sub_train(batch, all_teams, t_env, episode_num, self.mac, self.mixer, self.optimiser,
                           self.params,
                           intrinsic_rewards=intrinsic_rewards,
                           show_demo=show_demo, save_data=save_data, show_v=show_v, ec_buffer=ec_buffer)
        ...
        if hasattr(self.args, 'save_buffer') and self.args.save_buffer:
            if self.buffer.episodes_in_buffer - self.save_buffer_cnt >= self.args.save_buffer_cycle:
                if self.buffer.can_sample(self.args.save_buffer_cycle):
                    ...
                    self.sub_train(batch_tmp, all_teams, t_env, episode_num, self.mac, self.mixer,
                                   self.optimiser, self.params,
                                   intrinsic_rewards=intrinsic_rewards_tmp,
    ...
    def _update_targets(self, ec_buffer):
        ...
        if self.args.EA:
            self.EA_target_mixer.load_state_dict(self.EA_mixer.state_dict())
