...
class vdn_QLearner:
    ...
    def subtrain(self, batch: EpisodeBatch, t_env: int, episode_num: int, mac, save_buffer=False, imac=None,
                 timac=None):
        ...
        self.soft_target_mac.agent.train()
        self.predict_mac.agent.train()
        target_mac_out = self.target_mac.forward(batch, batch.max_seq_length, batch_inf=True)[:, 1:,
                         ...]
        soft_target_mac_out = self.soft_target_mac.forward(batch, batch.max_seq_length, batch_inf=True)[
                              :, 1:, ...]
