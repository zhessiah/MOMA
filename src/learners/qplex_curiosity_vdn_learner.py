...
class QPLEX_curiosity_vdn_Learner:
    def __init__(self, mac, scheme, logger, args, groups=None, vdn_mac=None):
        ...
        self.args = args
        self.mac = mac
        if self.args.LTSCG and self.args.ifVDNmac:
            self.vdn_mac = vdn_mac
            self.vdn_params = list(vdn_mac.parameters())
        ...
        self.mixer = None
        if self.args.LTSCG and self.args.ifVDNmac:
            self.vdn_learner = vdn_QLearner(vdn_mac, scheme, logger, args)
        else:
    ...
    def sub_train(self, batch: EpisodeBatch, episode_sample: EpisodeBatch, max_ep_t, t_env: int, episode_num: int, mac,
                  mixer,
                  optimiser, params,
                  intrinsic_rewards,
        ...
        if mixer is not None:
            if self.args.mixer == "dmaq_qatten":
                ...
                if self.args.graphmixer:
                    ...
                    ans_chosen, q_attend_regs, head_entropies, local_rewards, alive_agents_mask = self.mixer(
                        chosen_action_qvals,
        ...
        if self.args.LTSCG:
            loss = loss + gstate_loss + gobs_loss
        # if self.args.graphmixer:
    ...
    def train(self, batch: EpisodeBatch, episode_sample: EpisodeBatch, max_ep_t, t_env: int, episode_num: int,
              show_demo=False, save_data=None, show_v=False,
              ec_buffer=None):

        if self.args.LTSCG and self.args.ifVDNmac:
            intrinsic_rewards = self.vdn_learner.train(batch, t_env, episode_num, save_buffer=False, imac=self.vdn_mac,
                                                       timac=self.vdn_target_mac)
        else:
            intrinsic_rewards = self.vdn_learner.train(batch, t_env, episode_num, save_buffer=False, imac=self.mac,
                                                       timac=self.target_mac)
        if self.args.is_prioritized_buffer:
            masked_td_error, mask = self.sub_train(batch, episode_sample, max_ep_t, t_env, episode_num, self.mac,
            ...
            self.sub_train(batch, episode_sample, max_ep_t, t_env, episode_num, self.mac, self.mixer, self.optimiser,
                           self.params,
                ...
                if self.buffer.can_sample(self.args.save_buffer_cycle):
                    ...
                    if self.args.LTSCG and self.args.ifVDNmac:
                        intrinsic_rewards_tmp = self.vdn_learner.train(batch_tmp, t_env, episode_num, save_buffer=True,
                                                                       imac=self.vdn_mac, timac=self.vdn_target_mac)
                    else:
                        intrinsic_rewards_tmp = self.vdn_learner.train(batch_tmp, t_env, episode_num, save_buffer=True,
                                                                       imac=self.mac, timac=self.target_mac)

                    self.sub_train(batch_tmp, episode_sample, max_ep_t, t_env, episode_num, self.mac, self.mixer,
