...
class FastMAC:
    ...
    def forward(self, ep_batch, t, test_mode=False, batch_inf=False):
        ...
        if hasattr(self.args, 'use_individual_Q') and self.args.use_individual_Q:
            return agent_outs.view(ep_batch.batch_size, self.n_agents, -1), individual_Q.view(ep_batch.batch_size,
                                                                                              self.n_agents, -1)
        else:
            if batch_inf:
    ...
    def _build_inputs(self, batch, t, batch_inf):
        ...
        if batch_inf:
            ...
            if self.args.obs_agent_id:
                inputs.append(
                    th.eye(self.n_agents, device=batch.device).view(1, 1, self.n_agents, self.n_agents).expand(bs, t,
                                                                                                               -1, -1))
