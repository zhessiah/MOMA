...
class EpisodeRunner:
    ...
    def run(self, mac, test_mode=False, EA=False, phase=0, **kwargs):
        self.mac = mac
        self.reset()
        ...
        while not terminated:
            ...
            if self.args.Pareto and not test_mode:
                result = self.env.step(actions[0], MultiObject=True)

                # 检查返回值的数量
                try:
                    _, reward, terminated, _, env_info, fitness = result
                except:
                    print(result, "4to3_error")
                # reward, terminated, env_info, fitness = self.env.step(actions[0], MultiObject=MultiObject)
                
                # [delta_enemy, delta_deaths, delta_ally]
                for i in range(len(fitness)):
                    fitnesseslist[i+1] += fitness[i]
                fitnesseslist[0] += reward
            else:
                _, reward, terminated, truncated, env_info, _ = self.env.step(actions[0], MultiObject=False)
